...
class TestCommandParser(unittest.TestCase):
    ...
    def test_look_command(self):
        ...
        player = copy.copy(Constants.TEST_PLAYER)
        actual = CommandParser.parse_input(user_input, player)
        self.assertEqual(Constants.TEST_ROOM.desc, actual)
    ...
    def test_take_command_item_on_ground(self):
        ...
        player = copy.copy(Constants.TEST_PLAYER)
        actual = CommandParser.parse_input(user_input, player)
        expected = Constants.ITEM_TAKEN_STRING + Constants.TEST_ITEM_ON_GROUND.name + '.'
        self.assertEqual(expected, actual)
    ...
    def test_take_command_item_in_inventory(self):
        ...
        player = copy.copy(Constants.TEST_PLAYER)
        actual = CommandParser.parse_input(user_input, player)
        expected = Constants.ITEM_NOT_VISIBLE_STRING + Constants.TEST_ITEM_IN_INVENTORY.name + '.'
        self.assertEqual(expected, actual)
    ...
    def test_take_command_item_not_present(self):
        ...
        player = copy.copy(Constants.TEST_PLAYER)
        actual = CommandParser.parse_input(user_input, player)
        expected = Constants.ITEM_NOT_VISIBLE_STRING + Constants.TEST_ITEM_NOT_PRESENT.name + '.'
        self.assertEqual(expected, actual)

    def test_take_command_item_not_takeable(self):
        user_input = "take test ungettable"
        player = copy.copy(Constants.TEST_PLAYER_2)
        actual = CommandParser.parse_input(user_input, player)
        expected = Constants.ITEM_NOT_TAKEABLE_STRING + Constants.TEST_ITEM_NO_GET.name + '.'
        self.assertEqual(expected, actual)